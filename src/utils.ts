...
export async function queryPackageInfo(pkg: string): Promise<Record<string, any>> {
  const REQUEST_URL = `https://registry.npmjs.org/${pkg}`;

  const res = await fetch(REQUEST_URL);
  if (!res.ok) {
    throw new Error(`request to url '${REQUEST_URL}' failed with error code ${res.status}`);
  }
  ...
  for (const pkg of pkgs) {
    try {
      ...
      if (pkgInfo.versions[last(Object.keys(pkgInfo.versions))]?.types) {
        info(`'${pkg}' has its built-in types, skipping its installation of types`);
        continue;
      }
      ...
      if (
        pkgTypesInfo.versions[last(Object.keys(pkgTypesInfo.versions))]?.deprecated &&
